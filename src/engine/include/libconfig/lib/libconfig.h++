...
namespace libconfig {
    ...
    public:
        ...
        inline const char *getError() const { return (_error); }
    ...
    public:
        ...
        inline const char *c_str() const { return operator const char *(); }
        ...
        inline Setting &lookup(const std::string &path) const { return (lookup(path.c_str())); }

        Setting &operator[](const char *name) const;

        inline Setting &operator[](const std::string &name) const { return (operator[](name.c_str())); }
        ...
        inline bool lookupValue(const std::string &name,
                                unsigned long long &value) const { return (lookupValue(name.c_str(), value)); }
        ...
        inline void remove(const std::string &name) { remove(name.c_str()); }

        void remove(unsigned int idx);

        Setting &add(const char *name, Type type);

        inline Setting &add(const std::string &name, Type type) { return (add(name.c_str(), type)); }
        ...
        inline bool isGroup() const { return (_type == TypeGroup); }

        inline bool isArray() const { return (_type == TypeArray); }

        inline bool isList() const { return (_type == TypeList); }

        inline bool isAggregate() const { return (_type >= TypeGroup); }

        inline bool isScalar() const { return ((_type > TypeNone) && (_type < TypeGroup)); }
    ...
    public:
        ...
        inline bool operator!=(SettingIterator const &other) const { return (!operator==(other)); }
    ...
    public:
        ...
        inline bool operator!=(SettingConstIterator const &other) const { return (!operator==(other)); }
    ...
    public:
        ...
        inline void setAutoConvert(bool flag) { setOption(Config::OptionAutoConvert, flag); }

        inline bool getAutoConvert() const { return (getOption(Config::OptionAutoConvert)); }

        void setDefaultFormat(Setting::Format format);

        inline Setting::Format getDefaultFormat() const { return (_defaultFormat); }
        ...
        inline void readString(const std::string &str) { return (readString(str.c_str())); }

        void readFile(const char *filename);

        inline void readFile(const std::string &filename) { readFile(filename.c_str()); }

        void writeFile(const char *filename);

        inline void writeFile(const std::string &filename) { writeFile(filename.c_str()); }
        ...
        inline bool exists(const std::string &path) const { return (exists(path.c_str())); }
        ...
        inline bool lookupValue(const std::string &path,
                                unsigned long long &value) const { return (lookupValue(path.c_str(), value)); }
